...
def _parse_text_fallback(text: str) -> dict:
    ...
    tm = re.search(r'(\d{2}[/\.]\d{2}[/\.]\d{4})\s*(?:-\s*\d{2}[/\.]\d{2}[/\.]\d{4})?\s*tarih', text)
    if tm:
        result["islem_tarihi"] = tm.group(1).replace("/",".")
    else:
        tm2 = re.search(r'(\d{2}[/\.]\d{2}[/\.]\d{4})', text)
        if tm2:
            result["islem_tarihi"] = tm2.group(1).replace("/",".")

    # Alım nominal — "12.000 TL toplam nominal tutarlı alış"
    alim = re.search(r'([\d.]+)\s*TL\s*toplam\s*nominal\s*tutarlı\s*al[ıi][şs]', text)
    if alim:
        result["alim_toplam_nominal"] = alim.group(1)

    # Satım nominal — "0 TL toplam nominal tutarlı satış"
    satim = re.search(r'([\d.]+)\s*TL\s*toplam\s*nominal\s*tutarlı\s*sat[ıi][şs]', text)
    if satim:
    ...
    if result.get("alim_toplam_nominal") and result.get("satim_toplam_nominal"):
        try:
            ...
            net = a - s
            result["net_nominal"] = f"{net:,.0f}".replace(",",".")
        except Exception:
            pass

    # Fiyat — "15,06 - 15,10 TL fiyat aralığından" veya "XX,XX TL fiyat"
    fp = re.search(r'([\d.,]+)\s*(?:-\s*([\d.,]+)\s*)?TL\s*fiyat', text)
    if fp:
        if fp.group(2):
            result["fiyat"] = f"{fp.group(1)} - {fp.group(2)}"
        else:
            result["fiyat"] = fp.group(1)
