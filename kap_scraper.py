...
def fetch_main_api(session: requests.Session, start_date: date, end_date: date) -> list:
    ...
    Tüm bildirimleri çeker - Pay Alım Satım filtrelemesi sonradan yapılır.
    """
    url = f"{BASE_URL}/tr/api/disclosure/list/main"

    # Payload - Network > Payload'dan gördüğünüz yapıya göre
    # Tarih aralığı için birkaç farklı body formatını deneriz
    payloads = [
        {
            "disclosureClass": "ODA",
        ...
        # Boş body - tüm bugünkü bildirimleri getirir
        {},
    ]
            ...
            if resp.status_code == 200:
                ...
                if text.startswith("[") and len(text) > 10:
                    ...
                    if isinstance(data, list) and len(data) > 0:
                        print(f"  ✓ /list/main API çalıştı ({len(data)} kayıt), payload: {payload}")
                        return data
        except Exception as e:
            print(f"  payload {payload} hata: {e}")
            continue
...
def filter_pay_alim_satim(raw_items: list) -> list:
    ...
    for item in raw_items:
        # disclosureBasic içinde olabilir
        basic = item.get("disclosureBasic", item)
        title   = (basic.get("title") or basic.get("subject") or
                   basic.get("disclosureSubject") or "").lower()
        summary = (basic.get("summary") or "").lower()

        if "pay alım satım" in title or "pay alim satim" in title.replace("ı","i"):
            results.append(item)
        elif "pay alım satım" in summary:
            results.append(item)
...
def normalize_item(item: dict) -> dict:
    """API response'unu standart iç formata çevirir."""
    basic  = item.get("disclosureBasic", item)
    detail = item.get("disclosureDetail", {}) or {}

    disc_id = basic.get("disclosureId") or basic.get("id") or ""
    link = ""
    if disc_id:
        link = f"{BASE_URL}/tr/Bildirim/{disc_id}"

    # publishDate "02.04.2026 20:46" veya "02-04-2026 20:46" formatında gelebilir
    pub_date = basic.get("publishDate") or basic.get("disclosureDate") or ""

    return {
        "no":     str(basic.get("disclosureIndex") or basic.get("id") or ""),
        "tarih":  pub_date,
    ...
    if disc_id:
        try:
            ...
            if resp.status_code == 200 and resp.text.strip().startswith("{"):
                ...
                if related:
                    detail["ilgili_sirket"] = ", ".join(
                        [r.get("stockCode","") for r in related if r.get("stockCode")]
                    )
        except Exception:
            pass
