...
def normalize_item(item: dict) -> dict:
    ...
    # İlgili şirketler - summary alanından çek (en güvenilir)
    # relatedStocks alanı bazen string "ISKPL KZGYO", bazen liste, bazen dict listesi gelir
    related_raw = item.get("relatedStocks") or basic.get("relatedStocks") or []
    ilgili_list = []
    if isinstance(related_raw, str):
        # Boşlukla ayrılmış kodlar: "ISKPL KZGYO" veya tek kod "ISKPL"
        for code in related_raw.split():
        ...
        for r in related_raw:
            if isinstance(r, dict):
                ...
                if code and len(code) >= 3:
                    ilgili_list.append(str(code))
            elif isinstance(r, str) and len(r.strip()) >= 3:
...
def parse_related_stocks(related_raw) -> str:
    """relatedStocks alanını güvenli şekilde parse eder."""
    codes = []
    if isinstance(related_raw, str):
        for code in related_raw.split():
            if len(code) >= 3 and code.replace(".","").isupper():
                codes.append(code)
    elif isinstance(related_raw, list):
        for r in related_raw:
            if isinstance(r, dict):
                c = r.get("stockCode") or r.get("code") or r.get("memberCode") or ""
                if c and len(str(c)) >= 3:
                    codes.append(str(c))
            elif isinstance(r, str) and len(r.strip()) >= 3:
...
def fetch_disclosure_detail(session: requests.Session, disc_id: str, url: str) -> dict:
    """
    Bildirim detayını KAP JSON API'den çeker.
    Endpoint: POST /tr/api/disclosure/detail/{disc_id}
    veya GET  /tr/api/disclosure/{disc_id}
    """
    detail = {
    ...
    # ── Yöntem 1: /tr/api/disclosure/detail/{id} ──
    detail_endpoints = [
        f"{BASE_URL}/tr/api/disclosure/detail/{disc_id}",
        f"{BASE_URL}/tr/api/disclosure/{disc_id}",
        f"{BASE_URL}/tr/api/disclosure/content/{disc_id}",
    ]
    for ep in detail_endpoints:
        try:
            ...
            # İlgili şirketler
            rs = data.get("relatedStocks") or data.get("disclosureBasic", {}).get("relatedStocks") or []
            ilgili = parse_related_stocks(rs)
            if ilgili:
                detail["ilgili_sirket"] = ilgili

            # disclosureContent veya content içinde tablo verisi olabilir
            content_sections = (
                data.get("disclosureContentList") or
                data.get("contentList") or
                data.get("content") or []
            )
            if isinstance(content_sections, str):
                content_sections = [{"content": content_sections}]

            for section in (content_sections if isinstance(content_sections, list) else []):
                html_content = section.get("content") or section.get("htmlContent") or ""
                if not html_content:
                    continue
                soup = BeautifulSoup(html_content, "html.parser")
                _parse_detail_tables(soup, detail)
                if detail["alim_toplam_nominal"] or detail["gun_sonu_nominal"]:
                    break

            if detail["alim_toplam_nominal"] or detail["gun_sonu_nominal"] or detail["ilgili_sirket"]:
                return detail
        except Exception:
            continue

    # ── Yöntem 2: HTML sayfasını çek ve Next.js __NEXT_DATA__ JSON'unu parse et ──
    fetch_url = url or f"{BASE_URL}/tr/Bildirim/{disc_id}"
    try:
        resp = session.get(fetch_url, timeout=20)
        soup = BeautifulSoup(resp.text, "html.parser")

        # Next.js sayfa verisi - __NEXT_DATA__ script tagı içinde
        ...
        if next_data_tag:
            try:
                ...
                page_props = (next_data.get("props", {})
                              .get("pageProps", {}))
                disc_data  = (page_props.get("disclosure") or
                              page_props.get("disclosureDetail") or
                              page_props.get("data") or {})

                # relatedStocks
                rs = disc_data.get("relatedStocks") or []
                ilgili = parse_related_stocks(rs)
                if ilgili and not detail["ilgili_sirket"]:
                    detail["ilgili_sirket"] = ilgili

                # Content HTML
                content_list = (disc_data.get("disclosureContentList") or
                                disc_data.get("contentList") or [])
                for section in content_list:
                    html_c = section.get("content") or section.get("htmlContent") or ""
                    if html_c:
                        _parse_detail_tables(BeautifulSoup(html_c, "html.parser"), detail)
                        if detail["alim_toplam_nominal"]:
                            break
            except Exception:
                pass

        # Sayfanın tamamını da dene (SSR render edilmiş olabilir)
        _parse_detail_tables(soup, detail)

        # Regex fallback - metin içinden çek
        full_text = soup.get_text(separator=" ")
        if not detail["islem_tarihi"]:
            m = re.search(r"(\d{2}[./]\d{2}[./]\d{4})\s*[Ii]şlem\s*[Tt]arih", full_text)
            if m:
                detail["islem_tarihi"] = m.group(1)
        if not detail["fiyat"]:
            m = re.search(r"[Oo]rtalama\s+([\d.,]+)\s*(?:TL\s*)?fiyat", full_text)
            if m:
                detail["fiyat"] = m.group(1)
        if not detail["ilgili_sirket"]:
            # [ISKPL] formatı
            codes = re.findall(r"\[([A-Z0-9]{2,8})\]", full_text)
            if codes:
...
def _parse_detail_tables(soup, detail: dict):
    """
    BeautifulSoup nesnesinden Pay Alım Satım tablolarını parse eder.
    Kolon sırası: İşlem Tarihi | Alım | Satım | Net | GünBaşı | GünSonu |
                  SermayeOranıGünBaşı | SermayeOranıGünSonu | OyHaklarıGünSonu
    """
    for table in soup.find_all("table"):
        ...
        is_pay_table = any(k in header_text for k in [
            "nominal", "sermaye", "satım", "alım", "işlem tar", "gün sonu", "gun sonu"
        ])
        if not is_pay_table:
            # Başlık satırı ilk satırda olmayabilir - tüm hücrelere bak
            all_text = table.get_text(" ").lower()
            if not any(k in all_text for k in ["nominal tut", "sermaye oran", "gün sonu nominal"]):
                continue
        ...
        for tr in rows[1:]:
            ...
            if not any(cells):
                continue
            # En az 5 kolon olmalı
            if len(cells) >= 5:
                def pick(idx, fallback=""):
                    return cells[idx] if idx < len(cells) else fallback

                detail["islem_tarihi"]           = pick(0) or detail["islem_tarihi"]
                detail["alim_toplam_nominal"]    = pick(1)
                detail["satim_toplam_nominal"]   = pick(2)
                detail["net_nominal"]            = pick(3)
                detail["gun_basi_nominal"]       = pick(4)
                detail["gun_sonu_nominal"]       = pick(5)
                detail["sermaye_orani_gun_basi"] = pick(6)
                detail["sermaye_orani_gun_sonu"] = pick(7)
                detail["oy_haklari_gun_sonu"]    = pick(8)
                return  # İlk eşleşen tabloda dur
