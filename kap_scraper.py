...
def fetch_bildirim_pdf(session, disc_index: str, log_fn=print) -> dict:
    ...
    try:
        ...
        first_bytes = r.content[:50] if r.content else b""
        log_fn(f"    📄 BildirimPdf: HTTP {r.status_code} | "
               f"Type: {content_type} | "
               f"Size: {len(r.content)} | "
               f"Start: {first_bytes[:30]}")

        if r.status_code != 200 or len(r.content) < 500:
            log_fn(f"    ⚠ Yanıt çok kısa veya hatalı")
            return {}
        ...
        if b'%PDF' in r.content[:20]:
            ...
            # HTML gelmiş — ilk 200 karakteri logla
            text_preview = r.text[:200].replace("\n"," ").strip()
            log_fn(f"    ⚠ PDF değil → HTML parse deneniyor")
            log_fn(f"    📝 İlk 200 kar: {text_preview}")
