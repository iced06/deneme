...
def fetch_bildirim_pdf(session, disc_index: str, log_fn=print) -> dict:
    ...
    # ── Yöntem 2: Ek dosya — disclosure detail veya direkt dosya linki ──
    attachment_result = _try_fetch_attachment(session, disc_index, log_fn)
...
def _try_fetch_attachment(session, disc_index: str, log_fn=print) -> dict:
    """DKB bildirimlerinin ek dosyasını bul ve parse et."""

    # Bilinen KAP ek dosya URL formatları:
    # /tr/api/disclosure/file/{discIndex}
    # /tr/api/file/download/{fileId}  (fileId bilinmiyor)
    ...
    for url in attachment_urls:
        try:
            ...
            log_fn(f"    📎 Ek dosya ({url.split('/')[-2]}/{url.split('/')[-1]}): "
            ...
            log_fn(f"    ✗ Ek dosya hata: {e}")

    # Yöntem 2: disclosure detail API'den file listesi al
    try:
        ...
        r = session.get(detail_url, headers=headers_json, timeout=15)
        log_fn(f"    📋 Detail API: HTTP {r.status_code} | Size: {len(r.content)}")

        if r.status_code == 200:
            try:
                detail = r.json()
                # Dosya listesi çeşitli alanlarda olabilir
                for key in ["attachments","files","disclosureFiles","fileList"]:
                    files = detail.get(key) or []
                    if isinstance(files, list) and files:
                        log_fn(f"    📎 {len(files)} ek dosya bulundu ({key})")
                        for f_info in files:
                            if isinstance(f_info, dict):
                                fid = (f_info.get("fileId") or f_info.get("id") or
                                       f_info.get("uuid") or f_info.get("fileUuid") or "")
                            else:
                                fid = str(f_info)
                            if fid:
                                ...
                                try:
                                    ...
                                    if fr.status_code == 200 and b'%PDF' in fr.content[:20]:
                                        ...
                                        if result.get("alim_toplam_nominal") or result.get("gun_sonu_nominal"):
                                            log_fn(f"    ✓ Ek dosya ({fid[:12]}...) parse başarılı")
                                            return result
                                except Exception:
                                    pass

                # content alanında HTML olabilir
                content = detail.get("content") or detail.get("disclosureHtml") or ""
                if content and "nominal" in content.lower():
                    text_result = _parse_text_fallback(content)
                    if text_result.get("alim_toplam_nominal"):
                        return text_result

            except (json.JSONDecodeError, ValueError):
                # JSON değilse HTML olabilir
                if "nominal" in r.text.lower():
                    text_result = _parse_text_fallback(r.text)
                    if text_result.get("alim_toplam_nominal"):
                        return text_result
    except Exception as e:
...
def fetch_html_detail(session, disc_index: str, log_fn=print) -> dict:
    """HTML sayfasından tablo veya metin içeriği çek."""
    result = {"data_rows": [], "fiyat": "", "sirket_adi": "", "text_data": {}}
    try:
    ...
    if not result["data_rows"]:
        # Sayfanın metin içeriğini logla (debug)
        body_len = len(body)
