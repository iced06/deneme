...
def normalize_item(item: dict) -> dict:
    ...
    # disclosureId — UUID, dosya erişimi için gerekli olabilir
    disc_id = b.get("disclosureId") or ""

    # Tüm alanları debug için sakla
    all_keys = sorted(b.keys()) if isinstance(b, dict) else []
    ...
    return {
        ...
        "_api_keys":     ",".join(all_keys),  # debug: hangi alanlar var
        "_raw_basic":    b,  # debug: tüm veri
    }
...
def _parse_kap_pdf(pdf_bytes: bytes, log_fn=print) -> dict:
    ...
    try:
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
            ...
            # Önce all_text'i normalize et (satır sonlarını birleştir)
            clean_text = re.sub(r'\n+', '\n', all_text)
            # Satır içi kırılmaları birleştir (cümle ortasındaki \n)
            clean_text = re.sub(r'(?<=[a-zçğıöşüA-ZÇĞİÖŞÜ,])\n(?=[a-zçğıöşüA-ZÇĞİÖŞÜ0-9%])', ' ', clean_text)

            log_fn(f"    📝 PDF metin ({len(all_text)} kar): "
                   f"'nominal' in text: {'nominal' in all_text.lower()}, "
                   f"'alış' in text: {'alış' in all_text.lower() or 'alis' in all_text.lower()}")

            text_result = _parse_text_fallback(clean_text)
            if text_result:
                result.update(text_result)
                log_fn(f"    ✓ Metin parse: {text_result}")
            else:
                # Son çare: all_text'ten de dene (normalize etmeden)
                text_result2 = _parse_text_fallback(all_text)
                if text_result2:
                    result.update(text_result2)
                    log_fn(f"    ✓ Metin parse (raw): {text_result2}")
...
def _parse_text_fallback(text: str) -> dict:
    ...
    # TÜM whitespace'i (newline dahil) tek boşluğa çevir — regex eşlemesi için kritik
    text_flat = re.sub(r'\s+', ' ', text)

    # Tarih — "01/04/2026 - 01/04/2026 tarihinde" veya tek tarih
    tm = re.search(r'(\d{2}[/\.]\d{2}[/\.]\d{4})\s*(?:-\s*\d{2}[/\.]\d{2}[/\.]\d{4})?\s*tarih', text_flat)
    if tm:
        result["islem_tarihi"] = tm.group(1).replace("/",".")
    else:
        tm2 = re.search(r'(\d{2}[/\.]\d{2}[/\.]\d{4})', text_flat)
        if tm2:
            result["islem_tarihi"] = tm2.group(1).replace("/",".")

    # Alım nominal — "12.000 TL toplam nominal tutarlı alış"
    alim = re.search(r'([\d.]+)\s+TL\s+toplam\s+nominal\s+tutarl[ıi]\s+al[ıi][şs]', text_flat, re.IGNORECASE)
    if alim:
        result["alim_toplam_nominal"] = alim.group(1)

    # Satım nominal — "0 TL toplam nominal tutarlı satış"
    satim = re.search(r'([\d.]+)\s+TL\s+toplam\s+nominal\s+tutarl[ıi]\s+sat[ıi][şs]', text_flat, re.IGNORECASE)
    if satim:
        result["satim_toplam_nominal"] = satim.group(1)

    # Net nominal hesapla
    if result.get("alim_toplam_nominal") and result.get("satim_toplam_nominal"):
        try:
            a = float(result["alim_toplam_nominal"].replace(".","").replace(",","."))
            s = float(result["satim_toplam_nominal"].replace(".","").replace(",","."))
            net = a - s
            if net == int(net):
                result["net_nominal"] = f"{int(net):,}".replace(",",".")
            else:
                result["net_nominal"] = str(net)
        except Exception:
            pass

    # Fiyat — "15,06 - 15,10 TL fiyat aralığından"
    fp = re.search(r'([\d.,]+)\s*-\s*([\d.,]+)\s*TL\s*fiyat', text_flat)
    if fp:
        result["fiyat"] = f"{fp.group(1)} - {fp.group(2)}"
    else:
        fp2 = re.search(r'([\d.,]+)\s*TL\s*(?:fiyat|ortalama)', text_flat)
        if fp2:
            result["fiyat"] = fp2.group(1)

    # Sermaye oranı — "%0,161 sınırına" veya "% X,XX"
    serm = re.findall(r'%\s*([\d.,]+)', text_flat)
    if serm:
        result["sermaye_orani_gun_sonu"] = serm[-1]
        if len(serm) >= 2:
            result["sermaye_orani_gun_basi"] = serm[-2]
...
def fetch_bildirim_pdf(session, disc_index: str, log_fn=print,
                       disc_id: str = "", extra_file_ids: list = None) -> dict:
    """
    KAP detay verileri çekme — 3 yöntem:
    1) BildirimPdf → PDF parse (tablo veya metin)
    2) Disclosure detail API → JSON'dan content/attachments
    3) Ek dosya indirme
    """

    # ── Yöntem 1: BildirimPdf → PDF (tablo varsa harika, yoksa metin parse) ──
    ...
    try:
        ...
        log_fn(f"    📄 BildirimPdf: HTTP {r.status_code} | "
               f"Type: {content_type} | Size: {len(r.content)}")

        if r.status_code == 200 and len(r.content) > 500:
            if b'%PDF' in r.content[:20]:
                log_fn(f"    ✓ PDF formatı algılandı")
                pdf_result = _parse_kap_pdf(r.content, log_fn)
                # PDF'den tam veri geldiyse döndür
                if pdf_result.get("alim_toplam_nominal") or pdf_result.get("gun_sonu_nominal"):
                    return pdf_result
                # Sadece tarih geldiyse — ek dosyaya geç
                log_fn(f"    ⚠ PDF'den tam veri gelmedi — ek dosya aranıyor")
            else:
                html_result = _parse_html_response(r.text, log_fn)
                if html_result.get("alim_toplam_nominal"):
                    return html_result
    except Exception as e:
        log_fn(f"    ✗ BildirimPdf hata: {e}")

    # ── Yöntem 2: Ek dosya — HTML'deki file ID'leri + diğer yöntemler ──
    attachment_result = _try_fetch_attachment(session, disc_index, disc_id, log_fn,
                                              extra_file_ids=extra_file_ids or [])
    if attachment_result and attachment_result.get("islem_tarihi"):
        return attachment_result

    # Hiçbiri çalışmadıysa boş döndür
    return {}


def _try_fetch_attachment(session, disc_index: str, disc_id: str = "", log_fn=print,
                           extra_file_ids: list = None) -> dict:
    """DKB bildirimlerinin ek dosyasını bul ve parse et."""
    ...
    # ── ÖNCELİKLİ: HTML sayfasından bulunan dosya ID'leri ──
    if extra_file_ids:
        log_fn(f"    📎 HTML'den {len(extra_file_ids)} dosya ID deneniyor")
        for fid in extra_file_ids:
            try:
                file_url = f"{BASE_URL}/tr/api/file/download/{fid}"
                r = session.get(file_url, headers=headers_dl, timeout=20)
                log_fn(f"    📎 file/{fid[:16]}...: HTTP {r.status_code} | {len(r.content)}b")
                if r.status_code == 200 and len(r.content) > 1000 and b'%PDF' in r.content[:20]:
                    result = _parse_kap_pdf(r.content, log_fn)
                    if result.get("alim_toplam_nominal") or result.get("gun_sonu_nominal"):
                        log_fn(f"    ✓ Ek dosya parse başarılı!")
                        return result
            except Exception as e:
                log_fn(f"    ✗ {e}")
    ...
    # Denenecek URL'ler
    urls_to_try = []
    # disclosureId (UUID) varsa — en olası endpoint'ler
    if disc_id:
        urls_to_try.extend([
            # Dosya listesi endpoint'leri (JSON döndürmeli)
            f"{BASE_URL}/tr/api/disclosure/{disc_id}/attachments",
            f"{BASE_URL}/tr/api/disclosure/files/{disc_id}",
            f"{BASE_URL}/tr/api/disclosureFiles/{disc_id}",
            f"{BASE_URL}/tr/api/notification/{disc_id}/files",
            # Doğrudan dosya indirme (disc_id = fileId olabilir)
            f"{BASE_URL}/tr/api/file/download/{disc_id}",
            f"{BASE_URL}/tr/api/disclosure/file/{disc_id}",
        ])
    # discIndex bazlı
    urls_to_try.extend([
        f"{BASE_URL}/tr/api/disclosure/{disc_index}/attachments",
        f"{BASE_URL}/tr/api/disclosure/files/{disc_index}",
        f"{BASE_URL}/tr/api/disclosure/file/{disc_index}",
        f"{BASE_URL}/tr/api/disclosureFiles/{disc_index}",
    ])

    for url in urls_to_try:
        try:
            r = session.get(url, headers=headers_dl, timeout=20)
            short_url = "/".join(url.split("/")[-3:])
            ct = r.headers.get("Content-Type", "?")
            log_fn(f"    📎 {short_url}: HTTP {r.status_code} | {ct} | {len(r.content)}b")
            ...
            if r.status_code == 200 and len(r.content) > 1000:
                if b'%PDF' in r.content[:20]:
                    ...
                    # JSON yanıt olabilir — dosya listesi
                    try:
                        data = r.json()
                        log_fn(f"    📋 JSON yanıt: {list(data.keys()) if isinstance(data, dict) else type(data).__name__}")
                        # JSON'dan dosya ID'leri çıkar
                        file_ids = _extract_file_ids(data)
                        if file_ids:
                            log_fn(f"    📎 {len(file_ids)} dosya ID bulundu")
                            for fid in file_ids:
                                file_url = f"{BASE_URL}/tr/api/file/download/{fid}"
                                try:
                                    fr = session.get(file_url, headers=headers_dl, timeout=20)
                                    if fr.status_code == 200 and b'%PDF' in fr.content[:20]:
                                        result = _parse_kap_pdf(fr.content, log_fn)
                                        if result.get("alim_toplam_nominal") or result.get("gun_sonu_nominal"):
                                            log_fn(f"    ✓ Dosya {fid[:16]}... parse başarılı!")
                                            return result
                                except Exception:
                                    pass
    ...
    try:
        ...
        log_fn(f"    📋 Detail API ({disc_index}): HTTP {r.status_code} | {len(r.content)}b")

        if r.status_code == 200 and len(r.content) > 100:
            try:
                detail = r.json()
                log_fn(f"    📋 Detail keys: {list(detail.keys()) if isinstance(detail, dict) else '?'}")
                file_ids = _extract_file_ids(detail)
                if file_ids:
                    for fid in file_ids:
                        file_url = f"{BASE_URL}/tr/api/file/download/{fid}"
                        try:
                ...
                if content and "nominal" in content.lower():
                    return _parse_text_fallback(content)
            except (json.JSONDecodeError, ValueError):
                if "nominal" in r.text.lower():
                    return _parse_text_fallback(r.text)
    except Exception as e:
        log_fn(f"    ✗ Detail API hata: {e}")

    return {}


def _extract_file_ids(data) -> list:
    """JSON verisinden dosya ID'lerini çıkar."""
    ids = []
    if isinstance(data, dict):
        for key in ["attachments","files","disclosureFiles","fileList"]:
            items = data.get(key) or []
            if isinstance(items, list):
                for item in items:
                    if isinstance(item, dict):
                        fid = (item.get("fileId") or item.get("id") or
                               item.get("uuid") or item.get("fileUuid") or "")
                        if fid:
                            ids.append(str(fid))
                    elif isinstance(item, str) and len(item) > 10:
                        ids.append(item)
        # disclosureId kendisi de dosya olabilir
        did = data.get("disclosureId") or ""
    ...
    for table in soup.find_all("table"):
        for row in table.find_all("tr"):
            ...
            if (len(cells) >= 5 and
                ...
                if len(cells) >= 9:
                    mapped = _map_standard_row(cells)
                    if mapped.get("islem_tarihi"):
                        log_fn(f"    ✓ HTML tablo (BildirimPdf): {mapped['islem_tarihi']}")
                        return mapped

    # 2) Tablo bulunamadı → sayfanın tüm metninden parse et
    full_text = soup.get_text("\n", strip=True)
    if full_text:
        text_result = _parse_text_fallback(full_text)
        if text_result.get("islem_tarihi"):
            log_fn(f"    ✓ HTML metin parse (BildirimPdf): {text_result['islem_tarihi']}")
            ...
            for tbl in soup.find_all("table"):
                for row in tbl.find_all("tr"):
                    ...
                    if len(cells) >= 2:
                        ...
                        if ("ad soyad" in key or "ticaret" in key) and val:
                            text_result["sirket_adi"] = val
                            break
                        elif "bildirime konu" in key and "borsa" in key and val:
                            # Bu şirket adı değil bildirime konu şirket
                            pass
...
def fetch_html_detail(session, disc_index: str, log_fn=print) -> dict:
    """HTML sayfasından tablo, metin içeriği ve ek dosya linkleri çek."""
    result = {"data_rows": [], "fiyat": "", "sirket_adi": "", "text_data": {}, "file_ids": []}
    try:
        ...
        if r.status_code != 200:
            return result
        log_fn(f"    📝 HTML sayfa: {len(r.text)} bytes")
    except Exception:
        return result
    ...
    # ── HTML'den file/download linkleri çıkar ──
    file_ids = re.findall(r'/api/file/download/([a-f0-9]{20,})', html)
    if file_ids:
        result["file_ids"] = list(set(file_ids))
        log_fn(f"    📎 HTML'de {len(result['file_ids'])} dosya linki bulundu")

    # ÖDA tipi: HTML tabloda veri satırı ara
    for table in soup.find_all("table"):
    ...
    # DKB tipi: Tablo yoksa sayfanın metin içeriğinden parse et
    if not result["data_rows"]:
        body_len = len(body)
        has_tarih = bool(re.search(r'\d{2}[/\.]\d{2}[/\.]\d{4}', body))
        has_nominal = "nominal" in body.lower()
        log_fn(f"    📝 HTML metin: {body_len} kar, tarih:{has_tarih}, nominal:{has_nominal}")

        text_data = _parse_text_fallback(body)
        if text_data.get("islem_tarihi"):
            result["text_data"] = text_data
            log_fn(f"    📝 HTML metin parse: {text_data}")
        else:
            log_fn(f"    ⚠ HTML metin parse başarısız")
...
def fetch_and_enrich(session, disc: dict, log_fn=print) -> dict:
    ...
    # 1a: HTML tabloda veri varsa (ÖDA tipleri)
    ...
    api_keys = disc.get("_api_keys", "")
    log_fn(f"    🔑 disc_id: {disc_id} | keys: {api_keys[:80]}")

    # HTML'den bulunan file_ids'i de gönder
    file_ids_from_html = html.get("file_ids", [])
    pdf = fetch_bildirim_pdf(session, disc_index, log_fn,
                              disc_id=disc_id,
                              extra_file_ids=file_ids_from_html)

    if pdf and pdf.get("islem_tarihi") and pdf.get("alim_toplam_nominal"):
        # PDF'den tam veri geldi
        ...
        for key in ["islem_tarihi", "alim_toplam_nominal", "satim_toplam_nominal",
            ...
            if pdf.get(key):
                row[key] = pdf[key]
        log_fn(f"    ✓ PDF: {row.get('islem_tarihi','-')}")
        return row

    # ── Yöntem 3: HTML sayfasının metin içeriğinden parse (DKB fallback) ──
    text_data = html.get("text_data", {})
    if text_data.get("islem_tarihi"):
        for key in ["islem_tarihi", "alim_toplam_nominal", "satim_toplam_nominal",
                    "net_nominal", "fiyat", "sermaye_orani_gun_basi",
                    "sermaye_orani_gun_sonu"]:
            if text_data.get(key):
                row[key] = text_data[key]
        log_fn(f"    ✓ HTML metin: {row.get('islem_tarihi','-')} | "
               f"Alım:{row.get('alim_toplam_nominal','-')}")
        return row

    # ── Yöntem 4: BildirimPdf kısmi sonuç (sadece tarih vb.) ──
    if pdf and pdf.get("islem_tarihi"):
        for key in ["islem_tarihi", "alim_toplam_nominal", "satim_toplam_nominal",
                    "net_nominal", "gun_basi_nominal", "gun_sonu_nominal",
                    "sermaye_orani_gun_basi", "oy_haklari_gun_basi",
                    "sermaye_orani_gun_sonu", "oy_haklari_gun_sonu", "fiyat"]:
